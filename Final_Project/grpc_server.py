...
class ServiceHandler (spb_grpc.FridgeServiceServicer):
  ...
  def method (self, request, context):
    ...
    try:
      ...
      print ("Received request - type: {}".format(request.type))
      resp_msg = "order_received"
      
      # Now send response
      resp = spb.Response ()  # TODO: allocate the response object. Note it is empty  
...
def driver (port):
  ...
  try:
    ...
    spb_grpc.add_FridgeServiceServicer_to_server(handler, server)   # TODO Change
...
def main ():
  ...
  print("Demo program for the Auction Settler / Bank")
