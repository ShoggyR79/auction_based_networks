...
def driver (name, iters, vec_len, port, server_ip, aggression):
  ...
  try:
    ...
    for i in range (iters):
      ...
      log(f"Bid: price: {price}, duration: {duration}, resources: {resources}")
      req = spb.Request()
      ...
      _resp = stub.method (req)
      log(f"Bid Status: {_resp.bid_response.status}")
      if _resp.bid_response.status == "Won Bid":
...
def main ():
  ...
  driver (parsed_args.name, parsed_args.iters, parsed_args.veclen, parsed_args.port, parsed_args.server_ip, float(parsed_args.aggression))
