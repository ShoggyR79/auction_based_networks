...
class Settler(object):
  ...
  def accept_request(self, user, bid):
    self.queue.append((user, bid))
    log(f"Request submitted by {user} - duration: {bid.duration}, resources: {bid.resources}")

  def settle_requests(self):
    self.winners = []
    while len(self.queue) > 0:
      user, bid = self.queue.pop(0)
      can_schedule = True
      for r in bid.resources:
      ...
      if can_schedule:
        ...
        for r in bid.resources:
            load_balancer.schedule(r, user, bid.duration)
...
def timer_update():
  ...
  while True:
      ...
      with open("FCFS_requests_processed.txt", "a") as file:
      ...
      with condition:
        load_balancer.tick()
        settler.settle_requests()
        log("Requests settled")
  ...
  def method (self, request, context):
    ...
    try:
      ...
      if (request.type == spb.RequestType.ORDER) :
        ...
        settler.accept_request(user, bid)
        with condition:
          condition.wait()
          bid_resp = spb.BidResponse()
          if settler.is_winner(user):
